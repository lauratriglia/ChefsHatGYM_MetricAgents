...
def run_room_with_order(agent_order, training, model_path_dql, model_path_attack, output_folder):
    ...
    agent_constructors = [
        ...
        lambda room: AgentDQLCustomReward("DQL_def", train=training, log_directory=room.room_dir, verbose_console=False, model_path=model_path_attack, load_model=not training, reward_type="attack"),
        lambda room: LargerValue("LargerValue", log_directory=room.room_dir, verbose_console=False)
    ]
    agent_names = ["Random", "DQL_", "DQL_def", "LargerValue"]
...
if __name__ == "__main__":
    ...
    model_file_attack = os.path.join("src/agents/agent_metric/def", "dql_model.h5")
    ...
    final_dataset_path = os.path.join(output_base, "concatenated_all_permutations_dataset_def.csv")
    ...
    if concatenated_dataset is not None:
        scores_plot_path = os.path.join(output_base, "average_scores_analysis_def.png")
